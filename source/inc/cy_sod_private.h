...
#define cy_sod_app_log_err(ret_val,format,...)  printf ("[SOD] [Err:0x%"PRIx32"] "format" \r\n",ret_val,##__VA_ARGS__);
...
#define cy_sod_app_log_err(ret_val,format,...)  cy_log_msg (CYLF_MIDDLEWARE,CY_LOG_ERR,"[SOD] [Err:0x%"PRIx32"] "format" \r\n",ret_val,##__VA_ARGS__);
...
#define ENABLE_SOD_DBG_FEED_PRINT                         (0x0001)

#define SOD_PRINT_ARR_AS_CHAR(__X__,__Y__) \
{ \
    const unsigned char *temp = (const unsigned char *) __Y__; \
    (void*)temp; \
    cy_sod_app_log_info( "%s [0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X]", __X__,\
