...
cy_rslt_t cy_sod_init(cy_sod_config_params *config_params, cy_sod_t *handle)
{
    ...
    *handle = NULL;
    ...
    sod_context->sod_config_prms[1] = CY_SOD_SAMPLE_RATE_16000Hz;
    sod_context->sod_config_prms[2] = CY_SOD_FRAME_SIZE_MONO_SAMPLES;
        ...
        cy_sod_status_t *sod_status)
{
    ...
    {
        ret_val = CY_RSLT_SOD_BAD_ARG;
        cy_sod_app_log_info("Invalid arguments, %p %p %p", handle, data, sod_status);
        goto CLEAN_RETURN;
    }
    ...
    {
        ret_val = CY_RSLT_SOD_LICENSE_EXPIRED;
        cy_sod_app_log_err(ret_val, "License expired");
        goto CLEAN_RETURN;
    }

    *sod_status = CY_SOD_STATUS_INVALID;

#if (ENABLE_SOD_DEBUG & ENABLE_SOD_DBG_FEED_PRINT)
    SOD_PRINT_ARR_AS_CHAR("InputSOD",data);
#endif

#ifdef COMPONENT_PROFILER
    cy_sod_profile(SOD_PROFILE_CMD_START, NULL);
#endif
    ...
    ErrIdx |= speech_utils_sod_process(data, sod_context->ifx_sod_container,
            &sod_detect_status);
#else
    ErrIdx |= ifx_sod_process(data, sod_context->ifx_sod_container, &sod_detect_status);
#endif
    if (ErrIdx)
    {
...
#ifdef COMPONENT_PROFILER
    cy_sod_profile(SOD_PROFILE_CMD_STOP, NULL);
#endif
    ...
    if(true == sod_check_trigger)
    {
        ...
        {
            *sod_status = CY_SOD_STATUS_INPUT_DATA_PROCESSED;
        }
    }
    else
    {
